...
<template>
  <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50 shrink-0 flex items-center justify-end gap-3 rounded-b-2xl">
    ...
    <button @click="emit('submit', 'draft')" :disabled="isSubmitting"
      class="px-5 py-2.5 text-[10px] font-black uppercase tracking-widest text-indigo-600 bg-indigo-50 rounded-xl hover:bg-indigo-100 transition-all active:scale-[0.98] disabled:opacity-50">
      {{ isSubmitting ? '...' : 'Simpan Draft' }}
    </button>
    <button @click="emit('submit', 'pending')" :disabled="isSubmitting"
      class="px-6 py-2.5 text-[10px] font-black uppercase tracking-widest text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 active:scale-[0.98] disabled:opacity-50 flex items-center gap-2">
